...
const FAQ_ITEMS = [
  {
    ...
    answer:
      "Здесь собран общий банк: ручные депозиты, ручные выводы и автоматический результат по слотам.",
  },
  {
    question: "Почему в финансах появились операции из слотов?",
    answer:
      "Потому что приложение теперь связывает игру с общим банком. Когда вы ведёте записи по слотам, их результат автоматически учитывается в финансах.",
  },
  {
    question: "Что значат фильтры «Все / Ручные / Из слотов»?",
    answer:
      "Они переключают список операций. Можно смотреть всё вместе, только ваши ручные движения денег или только результат, который пришёл из слот-сессий.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "Это настройка приложения. Когда она включена, после сохранения форма оставляет слот и ставку, чтобы удобнее вносить серию похожих записей подряд.",
  },
  {
  ...
  {
    question: "Что означает блок «Влияние слотов на общий банк»?",
    answer:
      "Он связывает аналитику и финансы: показывает результат слотов, сумму депозитов, сумму выводов и общий итог банка в одном месте.",
  },
  {
    question: "Что есть в настройках?",
    answer:
      "Там собраны все настройки приложения: тема, валюта, быстрая запись слотов, экспорт данных и полный сброс. Они специально не дублируются в других разделах.",
  },
  {
    question: "Как работает тема приложения?",
    answer:
      "В настройках можно переключаться между тёмной и светлой темой. Выбранная тема сохраняется и восстанавливается при следующем открытии.",
  },
  {
    question: "Как работает экспорт данных?",
    answer:
      "Экспорт сохраняет ручные финансы, слот-сессии, пользовательские слоты, избранное и настройки в JSON-файл. Это ваша резервная копия.",
  },
  {
